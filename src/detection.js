...
export function testBox3Collision(a, b, contact = {}) {
  ...
  // ==========================
  const mtv = {
    distance: Number.MAX_VALUE, // Set current minimum distance (max float value so next value is always less)
  ...
  // [X Axis]
  if (!testAxisStatic(xAxis, a.min.x, a.max.x, b.min.x, b.max.x, mtv))
    return false;

  // [Y Axis]
  if (!testAxisStatic(yAxis, a.min.y, a.max.y, b.min.y, b.max.y, mtv))
    return false;

  // [Z Axis]
  if (!testAxisStatic(zAxis, a.min.z, a.max.z, b.min.z, b.max.z, mtv))
    return false;
