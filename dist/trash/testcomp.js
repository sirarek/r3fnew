...
import { Vector3, Matrix4, Box3, BoxHelper, Plane, Matrix3, PlaneHelper, MeshBasicMaterial, QuadraticBezier } from "three";
const Tst = props => {
  const ref = useRef();
  const ref2 = useRef();
  const refBox = useRef();
  ...
  const cp = new Plane(new Vector3(1, 0, -0.5), 1);
  const cp1 = new Plane(new Vector3(0.5, 0, -1), 1);
  useEffect(() => {
  ...
  }), props.ax && /*#__PURE__*/React.createElement("planeHelper", {
    size: 10,
    plane: props.ax === "y" ? cp : cp1,
    color: 0xff0000
  }));
};
export default Tst;