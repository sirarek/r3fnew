...
export const sendScreenshot = (threState, addScreenshot) => {
  ...
  const {
    height,
    width
  } = threState.scene.children.find(g => g.name === "room").children[0].children[0]["geometry"]["parameters"];
...
export const handleMessageExport = threeState => {
  ...
  exporter.parse(threeState.scene, gltf => save(new Blob([gltf], {
    type: "application/octet-stream"
  }), 'scene.glb'), () => console.log("error in glb"), {
    binary: true
  });
};
